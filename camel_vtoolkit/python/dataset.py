# Gererate trainng and test dataset for FR1/2/3 classifer and VH/VHH classifier
...
# Read into IGHV sequence dataset
v_seqs = pd.read_csv('../data/v_reference_extend.csv')



#### 1. Gererate trainng and test dataset for FR1/2/3 classifier
# Positive dataset
...
fr1_negative = pd.DataFrame(np.random.choice(letters, size=(10000, 26)), columns=list(range(1,27)))
fr2_negative = pd.DataFrame(np.random.choice(list(string.ascii_uppercase), size=(10000, 17)), columns=list(range(1,18)))
fr3_negative = pd.DataFrame(np.random.choice(list(string.ascii_uppercase), size=(10000, 39)), columns=list(range(1,40)))
...
fr3 = fr3.drop(["a69"], axis=1)


#### 2. Gererate trainng and test dataset for FR4 classifier
...
#### 2. Gererate trainng and test dataset for VH/VHH classifier
...
frs["type_adjusted"] = v_seqs["Type_adjusted"]